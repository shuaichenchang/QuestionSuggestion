    ...
    for db_id in dataset_by_db:
        ...
        for example in dataset_by_db[db_id]:
            ...
            for full_q_q in full_qs:
                ...
                positive_examples.append([[partial_q, query, ], 1])
            ...
            for example in dataset_by_db[db_id]:
                ...
                for full_q in all_query:
                    if full_q not in query_for_partial_q[partial_q] and random.random() < 1.0/args.positive_over_negative/len(all_query):
                        negative_exmaples.append([[partial_q,full_q],0])
                # else:
...
def batch_iter(data_set, batch_size,shuffle=False, seed=12345):
    ...
    for batch_id in range(batch_num):
        ...
        batch_examples.sort(key=lambda e: -len(e[0][0]))
        batch_examples_question=[[x[0][0],x[1]] for x in batch_examples]
        batch_examples_sql = [[x[0][1], x[1]] for x in batch_examples]

        yield (batch_examples_question,batch_examples_sql)
